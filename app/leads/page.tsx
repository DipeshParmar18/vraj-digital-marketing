...
export default function Page() {
  ...
  const load = async () => {
    const [d, c] = await Promise.all([
      supabase.from('leads').select('*, clients(name)').order('created_at', { ascending: false }),
  ...
  const save = async () => {
    setSaving(true)
    if (editing) await supabase.from('leads').update(form).eq('id', editing)
  ...
  return (
    <div>
      <div className="topbar">
        <div>
          <div style={{ fontWeight:700, color:'var(--text-primary)' }}>⚡ Leads & CRM</div>
        ...
        <div className="modal-overlay" onClick={e => e.target===e.currentTarget && setModal(false)}>
          <div className="modal">
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'1.25rem' }}>
              <h2 style={{ fontSize:'1rem', fontWeight:700 }}>{editing ? 'Edit' : 'Add'} Leads & CRM</h2>
