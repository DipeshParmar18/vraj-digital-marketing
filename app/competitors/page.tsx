...
export default function Page() {
  ...
  const load = async () => {
    const [d, c] = await Promise.all([
      supabase.from('reports').select('*, clients(name)').order('created_at', { ascending: false }),
  ...
  const save = async () => {
    setSaving(true)
    if (editing) await supabase.from('reports').update(form).eq('id', editing)
    else await supabase.from('reports').insert(form)
  ...
  return (
    <div>
      <div className="topbar">
        <div>
          <div style={{ fontWeight:700, color:'var(--text-primary)' }}>🌐 Competitor Tracker</div>
        ...
        <div className="modal-overlay" onClick={e => e.target===e.currentTarget && setModal(false)}>
          <div className="modal">
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'1.25rem' }}>
              <h2 style={{ fontSize:'1rem', fontWeight:700 }}>{editing ? 'Edit' : 'Add'} Competitor Tracker</h2>
