...
export default function Page() {
  ...
  const save = async () => {
    setSaving(true)
    if (editing) await supabase.from('campaigns').update(form).eq('id', editing)
    else await supabase.from('campaigns').insert(form)
  ...
  return (
    <div>
      <div className="topbar">
        <div>
          <div style={{ fontWeight:700, color:'var(--text-primary)' }}>🎯 Google Ads</div>
        ...
        <div className="modal-overlay" onClick={e => e.target===e.currentTarget && setModal(false)}>
          <div className="modal">
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'1.25rem' }}>
              <h2 style={{ fontSize:'1rem', fontWeight:700 }}>{editing ? 'Edit' : 'Add'} Google Ads</h2>
